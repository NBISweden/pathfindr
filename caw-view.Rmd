...
makelinks <- function(strings,sep='&') {
  for (i in 1:length(strings)) if (strings[i]!='') {
...
tableWrapper <- function(table) {
  if ('Existing_variation' %in% colnames(table)) {
    table$ExistingVaritaion=makelinks(table$ExistingVariation)
  }
...
if (length(mutect2_file)>0) {
  ...
  selection <- mutect2_table[
    keep,
    .(VARIANT_CLASS,Existing_variation=makelinks(Existing_variation),chr,start,end,
