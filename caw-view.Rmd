...
if (length(vep_Tstruct_file)>0) {
  ...
  if (nrow(tstruc)>0) {
    ...
    del_rm <- intersect(which(abs(manta_tumor_table[,altpos-start])>3e6),
                        grep('ablation',manta_tumor_table$Annotation))
    # Amplification: remove >5M for 'duplication' consequences,
    amp_rm <- intersect(which(abs(manta_tumor_table[,altpos-start])>5e6),
    ...
    selection <- unique(
      manta_tumor_table[, # <---- not using "keep"
...
if (length(vep_Nstruct_file)>0) {
  ...
  ## Make new table with predicted effects
    selection <- unique(
      manta_normal_table[, # <---- not using "keep"
                        .(SVTYPE,ID=THISID,chr,start,end,Swegen_count,
                          altchr,altpos,SVLEN=unlist(SVLEN),
                          Gene_Name,rank_score=0,rank_terms='',LOH='',Comment='',
                          Annotation,Annotation_Impact,Allele,
...
if (length(mutect2_file)>0) {
  ...
  impact=which(mutect2_table$IMPACT %in% c('HIGH','MODERATE','MODIFIER'))
...
if (length(strelka_snv_file) * length(strelka_indel_file)>0) {
  ...
  # Keep high/mod-relevance in final output
  impact=which(strelka_snvs$IMPACT %in% c('HIGH','MODERATE','MODIFIER'))
  # Also keep Clinvar-pathogenic
  ...
  # Keep high/mod-relevance in final output
  impact=which(strelka_indels$IMPACT %in% c('HIGH','MODERATE','MODIFIER'))
  # Also keep Clinvar-pathogenic
...
```{r, echo=FALSE,,fig.width=15,fig.height=10,warning=F}
...
if (length(haplotypecaller_N_file)>0) {
  ...
  # Keep high/mod-relevance in final output
  impact=which(haplotypecaller_table$IMPACT %in% c('HIGH','MODERATE','MODIFIER'))
  # Also keep Clinvar-pathogenic
