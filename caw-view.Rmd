...
if (project == 'BTB') {
  ...
  freec_Nratio_file <- files[grep(pattern = '[BNT].hg38.pileup.gz_normal_ratio.txt',files,perl = T)][1]
  ...
  # Manta structural variant files
  manta_tumor_file <- grep(pattern = ".*Eff/Manta_.*vs.*somaticSV.vcf.snpEff.ann.vcf$",files,value = T)
  manta_normal_file <- grep(pattern = ".*Eff/Manta_.*vs.*diploidSV.vcf.snpEff.ann.vcf$",files,value = T)[1]
  # Haplotype caller variant files
  haplotypecaller_T_file <- grep(pattern = ".*CADD/haplotypecaller.*[TR2][.].*AF.*vep.ann.vcf$",files,value = T)
  # haplotypecaller_T_file <- 
  #   grep(pattern = "^.*Annotation/VEP/haplotypecaller.*02.*vcf$",files,value = T)[1]
  haplotypecaller_N_file <- grep(pattern = ".*CADD/haplotypecaller.*[NB1][.].*AF.*vep.ann.vcf$",files,value = T)
  ...
  mutect2_file <- grep(pattern = ".*CADD/mutect2_.*AF.*vep.ann.vcf$",files,value = T)
  # snpEff + VEP Strelka snv file
  strelka_snv_file <- grep(pattern = ".*CADD/Strelka_.*_somatic_snvs.*AF.*vep.ann.vcf$",files,value = T)
  # snpEff + VEP Strelka indel file
  strelka_indel_file <- grep(pattern = ".*CADD/Strelka_.*_somatic_indels.*AF.*vep.ann.vcf$",files,value = T)
}
...
if (project=='BTB') {
  local_tumorgenes=fread('~/reports/reference_data/2020_gene_list_tere_toMarkus20200609.csv',key='Gene')[Gene!='',1:2]
...
if (project=='BTB') if (!is.na(freec_Tratio_file[1])) {
  ...
  for (s in 1:length(freec_Tratio_file)) {
    ...
    # Add percent of CNAs to sampleData
    ix= !temp$Chromosome %in% c('X','Y') & temp$CopyNumber != median(temp$CopyNumber)
    sampleData[[freec_cnv_file[s]]]=round(100*sum(ix)/length(ix))
  ...
  if (write_tables) fwrite(freec_cnv,file=paste0(csv_dir,'/',sampleData$name,'_freec_cnv.csv')) # write to file
...
if (length(ascat_Tratio_file)>0) {
  ...
  if (write_tables) fwrite(ascat_cnv,file=paste0(csv_dir,'/',sampleData$name,'_ascat_cnv.csv'))
...
if (length(manta_tumor_file)>0) if (!is.na(manta_tumor_file)) for (s in 1:length(manta_tumor_file)) {
  ...
  sampleData[[manta_tumor_file[s]]]=length(vcf)
...
if (!is.null(manta_tumor_table)) if (nrow(manta_tumor_table)>0) {
  ...
  if (nrow(selection)>0) {
    ...
    if (write_tables) fwrite(manta_tumor_selected[,-c('ANN','CSQ','Gene_ID')][grep('_gene',rank_terms)],file=paste0(csv_dir,'/',sampleData$name,'_manta_tumor.csv'))
    tableWrapper(manta_tumor_selected[,-c('ANN','CSQ','Gene_ID')][grep('_gene',rank_terms)])
  } 
}
...
if (project=='BTB') {
  ...
  allpass=NULL
  vcfs=list()
  if (length(manta_normal_file)>0) if (!is.na(manta_normal_file)) for (s in 1:length(manta_normal_file)) {
  ...
  if (!is.null(manta_normal_table)) if (nrow(manta_normal_table)>0) {
    ...
    if (nrow(selection)>0) {
      ...
      if (write_tables) fwrite(manta_normal_selected[,-c('ANN','CSQ','Gene_ID')][grep('_gene',rank_terms)],file=paste0(csv_dir,'/',sampleData$name,'_manta_normal.csv'))
      tableWrapper(manta_normal_selected[,-c('ANN','CSQ','Gene_ID')][grep('_gene',rank_terms)])
    } 
  }
}
```
...
#     }
    ...
    if (length(vcf)>0) {
      ...
      sampleData[[mutect2_file[s]]]=sum(mutations$AFreq>0.1)
  ...
  if (write_tables) fwrite(mutect2_selected[grep('_gene',rank_terms)],file=paste0(csv_dir,'/',sampleData$name,'_mutect2_tumor.csv'))


  tableWrapper(mutect2_selected[,-c('cumstart','cumend','DOMAINS')][grep('_gene',rank_terms)])
}
...
if (length(strelka_snv_file)>0  & length(strelka_indel_file)>0) {
  ...
  if (write_tables) fwrite(strelka_selected[grep('_gene',rank_terms)],file=paste0(csv_dir,'/',sampleData$name,'_strelka_tumor.csv'))
  tableWrapper(strelka_selected[,-c('cumstart','cumend','DOMAINS')][grep('_gene',rank_terms)])
}
...
if (exists('haplotypecaller_N_file')) {
  ...
  selection=haplotypecaller_table[!Consequence %in% c('intron_variant','intergenic_variant'),-c('CSQ')] # CSQ field not needed after parsing/merging the annotations
  ...
  if (write_tables) fwrite(haplotypecaller_selected[grep('_gene',rank_terms)],file=paste0(csv_dir,'/',sampleData$name,'_haplotypecaller.csv'))
  ix=haplotypecaller_selected$sample==strsplit(basename(haplotypecaller_files[1]),'[.]')[[1]][1] # first file is the normal
  tableWrapper(haplotypecaller_selected[ix][,-c('cumstart','cumend','DOMAINS')][grep('_gene',rank_terms)])
...
if (project=='BTB') tableWrapper(haplotypecaller_selected[,-c('cumstart','cumend','DOMAINS')][grep('_gene',rank_terms)])
```
...
cat('Manta somatic')
try( {
  t=manta_tumor_selected[rank_score>3]
...
if (exists('manta_normal_selected')) try({
  t=manta_normal_selected[rank_score>3]
  t$Gene_Name[nchar(t$Gene_Name)>100]='many'
  if (nrow(t)>0){
    t=unique(data.table(Sample=t$sample,
                 Gene=t$Gene_Name,
...
if (F) if (write_tables) try({
  ...
  #x[,1]=paste0('chr',x[,1])
  #save(x,file =  paste0(csv_dir,'/',sampleData$name,'_forBamfile.Rdata'))
  write.table(
    x,
    file = paste0(csv_dir,'/',sampleData$name,'_forBamfile.bed'
    ),
    sep = '\t',row.names = F,col.names = F,quote=F)
}, silent=T)

save.image(paste0(csv_dir,'/',sampleData$name,'_ws.Rdata'))
fwrite(sampleData,file = paste0(csv_dir,'/',sampleData$name,'_sampleData.csv'))
