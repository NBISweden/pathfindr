...
if (project=='BTB') {
  ...
  if (!is.null(manta_normal_table)) if (nrow(manta_normal_table)>0) {
    ...
    if (nrow(selection)>0) {
      ...
      if (write_tables) fwrite(manta_normal_selected[,-c('ANN','CSQ','Gene_ID')][rank_score>3],file=paste0(csv_dir,'/',sampleData$name,'_manta_normal.csv'))
      tableWrapper(manta_normal_selected[,-c('ANN','CSQ','Gene_ID')][rank_score>3])
    } 
  }
}
```
...
#     }
  ...
  if (write_tables) fwrite(mutect2_selected[rank_score>3],file=paste0(csv_dir,'/',sampleData$name,'_mutect2_tumor.csv'))
...
if (length(strelka_snv_file)>0  & length(strelka_indel_file)>0) {
  ...
  if (write_tables) fwrite(strelka_selected[rank_score>3],file=paste0(csv_dir,'/',sampleData$name,'_strelka_tumor.csv'))
  tableWrapper(strelka_selected[,-c('cumstart','cumend','DOMAINS')][rank_score>3])
}
...
if (exists('haplotypecaller_N_file')) {
  ...
  if (write_tables) fwrite(haplotypecaller_selected[rank_score>3],file=paste0(csv_dir,'/',sampleData$name,'_haplotypecaller.csv'))
...
if (project=='BTB') tableWrapper(haplotypecaller_selected[,-c('cumstart','cumend','DOMAINS')][rank_score>3])
```
