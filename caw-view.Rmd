...
# control freec files
freec_Tratio_file <- files[grep(pattern = '[TR].hg38.pileup.gz_ratio.txt',files,perl = T)][2]
freec_Nratio_file <- files[grep(pattern = '[TR].hg38.pileup.gz_normal_ratio.txt',files,perl = T)][2]
freec_Tbaf_file <- files[grep(pattern = '[TR].hg38.pileup.gz_BAF.txt',files,perl = T)][2]
freec_Nbaf_file <- files[grep(pattern = '[BN].hg38.pileup.gz_BAF.txt',files,perl = T)][2]
freec_info_file <- files[grep(pattern = 'hg38.pileup.gz_info.txt',files,perl = T)][2]
freec_cnv_file <- files[grep(pattern = "^.*[TR]\\.hg38\\.pileup\\.gz_CNVs$",files,perl = T)][2]
# Ascat files
...
vep_Tstruct_file <- grep(pattern = "^.*Annotation/.*/Manta.*vs.*somaticSV.vcf.*snpEff.*ann.*vcf$",files,value = T)[1]
vep_Nstruct_file <- grep(pattern = "^.*Annotation/.*/Manta.*vs.*diploidSV.vcf.*snpEff.*ann.*vcf$",files,value = T)[1]
# Haplotype caller variant files
# haplotypecaller_T_file <- grep(pattern = "^.*Annotation/VEP/haplotypecaller.*[TR]\\.vcf.*vcf$",files,value = T)[1]
...
mutect2_file <- grep(pattern = "^.*Annotation/VEP/mutect2_.*vcf$",files,value = T)
...
tableWrapper <- function(table) {
  ...
  if ('Gene_Name' %in% colnames(table)) {
    table$Gene_Name=makelinks(table$Gene_Name)
  }
  ...
  htmlTable(table, col.rgroup = c("none", "#F7F7F7"))
}
...
alltumorgenes=unique(c(tumorgenes$`Gene Symbol`,oncokb_tumorgenes$`Hugo Symbol`,local_tumorgenes$Gene))
...
alltsg=union(tumorgenes[grep('TSG',`Role in Cancer`),`Gene Symbol`],oncokb_tumorgenes[`OncoKB TSG`=='YES',`Hugo Symbol`])
hotspots_inframe=fread('~/reports/reference_data/hotspots_v2_inframe.csv')
hotspots_snv=fread('~/reports/reference_data/hotspots_v2_snv.csv')
...
cnvs=NULL
freec_cnv=NULL
if (!is.na(freec_Tratio_file) &
    !is.na(freec_Nratio_file) &
    !is.na(freec_Tbaf_file) &
    !is.na(freec_Nbaf_file)) {
  # tumor log ratio
...
if (exists('tratio')) {
  ...
  # add smoothed
  #g=g+geom_line(aes(x=cumpos,y=tratio),stat="identity",colour="green",size=0.1,data=binned)
...
if (exists('ascat_tratio')) {
  ...
  # add smoothed
  #g=g+geom_line(aes(x=cumpos,y=tratio),stat="identity",colour="green",size=0.1,data=ascat_binned)
  ...
  if (nrow(tstruc)>0) {
    ...
    if (nrow(selection)>0) {  
      ...
      for (gene in unique(selection$Gene_Name)) if (gene!='') {
        ...
        if (any(gene %in% oncokb_tumorgenes$`Hugo Symbol`)) {
    ...
    tableWrapper(manta_tumor_selected[rank_score>2])
...
if (length(vep_Nstruct_file)>0) {
  ...
  if (nrow(selection)>0) {  
    ...
    tableWrapper(manta_normal_selected[rank_score>2])
  }
}
}
...
if (length(mutect2_file)>0) {
  #mutect2_file='~/Data/BTBdata/mutect2_2018-10088-02_vs_2018-10088-01.AF.snpEff.ann.vep.ann.vcf'
  vcf=readVcf(file = mutect2_file,genome ='GRCh38')
  #vcf=vcf[rowRanges(vcf)$FILTER=='PASS'] # this does not work with the new GATK
  ...
  if (length(vcf)>0) {
    ...
    tmuts$alt=as.data.table(alt(vcf))[, .(values = list(value)), by = group][,values]
    ...
    # ix=grep(';',tmuts$names)
    # ids=data.table(ix,id=strsplit(tmuts$names[ix],';'))
    # ids$counts=unlist(lapply(ids$id,function(x) return(max(snptable[x,value],na.rm=T))))
    # ids$counts[is.infinite(ids$counts)]=0
    # tmuts$Swegen_count[ix]=ids$counts
    # # Also double check Swegen in case pos in reference but rsID or not properly matched in data
    # pos=paste0('chr',tmuts$chr,':',tmuts$start,'_',tmuts$ref,'/',tmuts$alt)
    # snps=snptable[c(pos)][!is.na(value)] # extract from reference[those present]
    # if (nrow(snps)>0) tmuts$Swegen_count[match(snps$name,pos)]=snps$value # put
    # # Some have multiple alt alleles
    # ix=grep(',',tmuts$alt)
    # for (i in ix) {
    #   alts=tmuts$alt[[i]]
    #   snps=snptable[paste0('chr',tmuts$chr[i],':',tmuts$start[i],'_',tmuts$ref[i],'/',alts)]
    #   tmuts$Swegen_count[i]=max(c(0,snps$value),na.rm = T)
    # }
    ...
    tmuts$AFreq=sapply(g$AF[,'TUMOR'], "[", 1)
    tmuts$DP=sapply(g$AD[,'TUMOR'], sum)
    tmuts$AD=sapply(g$AD[,'TUMOR'], "[[", 2)
    tmuts$DPn=sapply(g$AD[,'NORMAL'], sum)
    tmuts$ADn=sapply(g$AD[,'NORMAL'], "[[", 2)
    tmuts$reads=''
    tmuts$reads[tmuts$AD<5]='<5'
    tmuts$reads[tmuts$AD>=5]='≥5'
    
    ## Annotate by cosmic (for retaining non PASS hotspots, which is not done yet)
    key=paste0(tmuts$chr,':',tmuts$start,'-',tmuts$end)
    ...
    tmuts=cbind(tmuts,as.data.table(info(vcf))[,.(CSQ)])
    ...
    annotation_table=matrix(data = NA,nrow = length(unlist(tmuts$CSQ)),ncol = length(vep_header)+1)
    ...
    tmuts$N=as.character(1:nrow(tmuts))
    mutect2_table=merge(tmuts,annotation_table,by='N')
    ...
    if (nrow(selection)>0) {  
      ...
      ix <-
        ...
        temp %in% paste(hotspots_inframe[,Hugo_Symbol],hotspots_inframe[,Amino_Acid_Position])  ## Warning: Exact match used with inframes
      if (any(ix)) {
        selection$rank_score[ix]=selection$rank_score[ix]+2
...
if (!is.na(strelka_snv_file)  & !is.na(strelka_indel_file)) {
  ...
  for (this_ref in unique(tmuts$ref)) for (this_alt in unique(tmuts[ref==this_ref,alt])) {
    which_ones=tmuts$ref==this_ref & tmuts$alt==this_alt
    refcounts=as.data.table(data.frame(g[[paste0(this_ref,'U')]]))
    altcounts=as.data.table(data.frame(g[[paste0(this_alt,'U')]]))
    tmuts$AD[which_ones]=altcounts[which_ones,TUMOR.1]
    tmuts$ADn[which_ones]=altcounts[which_ones,NORMAL.1]
    tmuts$AFreq[which_ones]=altcounts[which_ones,TUMOR.1] /
  ...
  # Annotation table then concatenated 
  tmuts$N=as.character(1:nrow(tmuts))
  strelka_snvs=merge(tmuts,annotation_table,by='N')
  ...
  tmuts$ADn=altcounts$NORMAL.1
  tmuts$AD=altcounts$TUMOR.1
  tmuts$AFreq=altcounts$TUMOR.1 / (altcounts$TUMOR.1 + refcounts$TUMOR.1)
  ...
  # Annotation table then concatenated 
  tmuts$N=as.character(1:nrow(tmuts))
  strelka_indels=merge(tmuts,annotation_table,by='N')
...
if (exists('mutect2_table')) {
  ...
  g=ggplot(data = mutect2_table[mutect2_table$Callers=='Mutect 2 and Strelka',],
           aes(x=cumstart,y=AFreq,fill=type,shape=type,col=reads))+
    geom_point(alpha=1)+
  ...
  # add gene labels
  temp=mutect2_table[mutect2_table$Callers=='Mutect 2 and Strelka' & mutect2_table$Amino_acids!='',]
...
if (length(haplotypecaller_N_file)>0) {
  #haplotypecaller_N_file='~/Data/BTBdata/haplotypecaller_2018-10088-01.AF.snpEff.ann.vep.ann.vcf'
  #haplotypecaller_T_file='~/Data/BTBdata/haplotypecaller_2018-10088-02.AF.snpEff.ann.vep.ann.vcf'
  ...
  nmuts$alt=''
  #nmuts$alt2=''
  nmuts$alt=as.data.table(alt(vcf))[, .(values = list(value)), by = group][,values]
  ...
  nmuts$AFreq=apply(X = ad[,2:3],MARGIN = 1,FUN = max)/nmuts$DP
  ...
  # Some have multiple rsIDs
  ix=grep(';',nmuts$names)
  ids=data.table(ix,id=strsplit(nmuts$names[ix],';'))
  ids$counts=unlist(lapply(ids$id,function(x) return(max(snptable[x,value],na.rm=T))))
  ids$counts[is.infinite(ids$counts)]=0
  nmuts$Swegen_count[ix]=ids$counts

  # Also double check Swegen in case pos in reference but rsID or not properly matched in data
  pos=paste0('chr',nmuts$chr,':',nmuts$start,'_',nmuts$ref,'/',nmuts$alt)
  snps=snptable[c(pos)][!is.na(value)] # extract from reference[those present]
  if (nrow(snps)>0) nmuts$Swegen_count[match(snps$name,pos)]=snps$value # put
  # # Some have multiple alt alleles
  # ix=grep(',',nmuts$alt)
  # alts=data.table(ix,alt=nmuts$alt[ix])
  # alts$keys=
  # alts$counts=unlist(lapply(alts$pos,
  #                           function(x) return(max(snptable[x,value],na.rm=T))))
  ...
  nmuts$type[nchar(nmuts$type)>3]=NA
  ...
  ## Annotate by cosmic
  key=paste0(nmuts$chr,':',nmuts$start,'-',nmuts$end)
  counts=cbind(cosmic_coding[key,value],cosmic_noncoding[key,value],0)
  max_=apply(counts,1,max,na.rm=T)
  nmuts$cosmic_n=max_
  
  # "info"" annotations also need to be added
  nmuts=cbind(nmuts,as.data.table(info(vcf))[,.(CSQ)])
  ## get snpEff headers
  #snpeff_header=strsplit(info(header(vcf))['ANN',][,3],'ions: \'')[[1]][2]
  #snpeff_header=strsplit(snpeff_header,' \\| ')[[1]]
  ## get VEP headers
  ...
  # VEP annotation is put in annotation_table
  annotation_table=matrix(data = NA,nrow = length(unlist(nmuts$CSQ)),ncol = length(vep_header)+1)
  colnames(annotation_table)=c('N',vep_header)
  row=1
  ...
  # Annotation table then concatenated 
  nmuts$N=as.character(1:nrow(nmuts))
  haplotypecaller_table=merge(nmuts,annotation_table[,colnames(annotation_table)!='AF'],by='N')
  
  # Due to size: remove intron/intergenic variants
  haplotypecaller_table=haplotypecaller_table[!Consequence %in% c('intron_variant','intergenic_variant')]
  ...
  # snps=snptable[ids][!is.na(value)] # now counts for these will need to be added to our table
  # 
  # snps$ix=match(snps$name,haplotypecaller_table$Existing_variation) # these ones are easily found, as not 2 ids in same row
  # haplotypecaller_table$Swegen_count[snps[!is.na(ix),ix]]=snps$value[snps[!is.na(ix),value]]
  # snps=snps[is.na(ix),1:2] # the remaining to check with grep
  # 
  # if (nrow(snps)>0) for (i in 1:nrow(snps)) {
  #     #cat(i,'\n')
  #     pattern=paste0(snps$name[i])
  #     ix=grep(pattern,haplotypecaller_table$Existing_variation)
  #     if (length(ix)>0) for (j in ix) {
  #       ids=strsplit(haplotypecaller_table$Existing_variation[j],'&')[[1]]
  #       #cat(ids,'\n')
  #       if (pattern %in% ids) haplotypecaller_table$Swegen_count[j]=snps$value[i]
  #     }
  # }
  # 
  # # Also double check Swegen in case pos in reference but rsID or not properly matched in data
  # pos=paste0('chr',haplotypecaller_table$chr,':',haplotypecaller_table$start,'_',haplotypecaller_table$ref,'/',haplotypecaller_table$alt) # the ID in case alt1 in reference
  # snps=snptable[c(pos)][!is.na(value)] # extract from reference[those present]
  # if (nrow(snps)>0) haplotypecaller_table$Swegen_count[match(snps$name,pos)]=snps$value # put
  
  # # Re-filter on counts
  #haplotypecaller_table=haplotypecaller_table[Swegen_count<5] <-- removed until we have validated
  
 # Keep everything annotated to a tumor gene
  in_tumorgene=which(haplotypecaller_table$SYMBOL %in% alltumorgenes)
  # Keep high/mod-relevance in final output
  ...
  # and everything that is >1 in cosmic
  in_cosmic=which(haplotypecaller_table$cosmic_n>1)
  

  keep_haplotypecaller <- sort(unique(c(impact,clinvar_pathogenic,polyphen_damaging,sift_deleterious,in_tumorgene,in_cosmic)))
  selection <- haplotypecaller_table[keep_haplotypecaller,
                                     .(VARIANT_CLASS,Existing_variation,chr,start,end,
                                       ...
                                       Consequence,IMPACT,CLIN_SIG,PolyPhen,SIFT,
                                       MOTIF_NAME, MOTIF_POS, HIGH_INF_POS, MOTIF_SCORE_CHANGE,
                                       ref,alt,DP,DP_t,RD,RD_t,AD,AD_t,AD2,AD2_t,
                                       CANONICAL,Protein_position)
                                     ]
  ...
  tableWrapper(haplotypecaller_selected)
...
if (exists('mutect2_selected')) { 
  t=head(mutect2_selected,50)
  if (nrow(t)>0){ 
    t=data.table(Patient=sampleData$name,
                 Somatic='somatic',
                 Gene=t$SYMBOL,
...
```{r}
#cat('HaplotypeCaller')
t=head(haplotypecaller_selected,50)
if (nrow(t)>0){ 
  t=data.table(Patient=sampleData$name,
               Somatic='germline',
               Gene=t$SYMBOL,
...
#cat('Manta somatic')
try( {
  t=head(manta_tumor_selected,50)
  if (nrow(t)>0){ 
    t=data.table(Patient=sampleData$name,
                 Somatic='somatic',
                 Gene=t$Gene_Name,
...
#cat('Manta germline')
try({
  t=head(manta_normal_selected,50)
  if (nrow(t)>0){ 
    t=data.table(Patient=sampleData$name,
                 Somatic='germline',
                 Gene=t$Gene_Name,
...
writeVcf(obj = vrA[vrA %in% vrB],
         filename = paste0('/castor/project/proj/reports/vcf_files/',sampleData$name,'_forPatterns.vcf'))
...
try({
  x = unique(rbind(
      ...
      haplotypecaller_table[keep_haplotypecaller,.(chr,start,end)])
    )
  x[,1]=paste0('chr',x[,1])
  save(x,file =  paste0('/castor/project/proj/reports/bed_files/',sampleData$name,'_forBamfile.Rdata'))
  write.table(
    x,
    file = paste0('/castor/project/proj/reports/bed_files/',sampleData$name,'_forBamfile.bed'
    ),
    sep = '\t',row.names = F,col.names = F,quote=F)
}, silent=T)
#
# export data tables for downstream analysis

#fwrite(freec_cnv,file=paste0('/castor/project/proj/reports/csv_files/',sampleData$name,'_freec_cnv.csv'))
fwrite(ascat_cnv,file=paste0('/castor/project/proj/reports/csv_files/',sampleData$name,'_ascat_cnv.csv'))
fwrite(manta_normal_selected,file=paste0('/castor/project/proj/reports/csv_files/',sampleData$name,'_manta_normal.csv'))
fwrite(manta_tumor_selected,file=paste0('/castor/project/proj/reports/csv_files/',sampleData$name,'_manta_tumor.csv'))
fwrite(mutect2_selected,file=paste0('/castor/project/proj/reports/csv_files/',sampleData$name,'_mutect2_tumor.csv'))
fwrite(strelka_selected,file=paste0('/castor/project/proj/reports/csv_files/',sampleData$name,'_strelka_tumor.csv'))
fwrite(haplotypecaller_selected,file=paste0('/castor/project/proj/reports/csv_files/',sampleData$name,'_haplotypecaller_normal.csv'))
...
```{r old_plot_function}  
  chromplot=function(thischr) if (T) {
    ...
    if (exists('mutect2_table')) if (nrow(mutect2_table)>0) {
      ...
      if (nrow(temp)>0) {
        ...
        g=g+geom_label_repel(data = temp[temp$Amino_acids!='',],
                             mapping = aes(x=start,y=AFreq -0.5,label=paste(SYMBOL,Protein_position,Amino_acids)),
    ...
    #genes!
    temp=allgenes[name %in% alltumorgenes & chr==thischr]
