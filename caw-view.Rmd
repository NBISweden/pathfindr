...
haplotypecaller_T_file <- grep(pattern = ".*haplotypecaller.*[TR].*AF.*vcf$",files,value = T)
# haplotypecaller_T_file <- 
#   grep(pattern = "^.*Annotation/VEP/haplotypecaller.*02.*vcf$",files,value = T)[1]
haplotypecaller_N_file <- grep(pattern = ".*haplotypecaller.*[NB].*AF.*vcf$",files,value = T)
...
mutect2_file <- grep(pattern = ".*mutect2_.*AF.*vcf$",files,value = T)
# snpEff + VEP Strelka snv file
strelka_snv_file <- grep(pattern = ".*Strelka_.*_somatic_snvs.*AF.*vcf$",files,value = T)
# snpEff + VEP Strelka indel file
strelka_indel_file <- grep(pattern = ".*Strelka_.*_somatic_indels.*AF.*vcf$",files,value = T)
...
if (!is.na(freec_Tratio_file[1])) {
  ...
  cnvs$length=paste(round((cnvs$end-cnvs$start)/1e6,2),'M')
  cnvs$rank_score <- 0
  cnvs$rank_terms <- ''
  cnvs$cancer_genes <- ''
...
if (length(ascat_Tratio_file)>0) {
  ...
  cnvs$length=paste(round((cnvs$endpos-cnvs$startpos)/1e6,2),'M')
  cnvs$rank_score <- 0
  cnvs$rank_terms <- ''
  cnvs$cancer_genes <- ''
...
if (nrow(manta_normal_table)>0) {
  ...
  if (nrow(selection)>0) {
    ...
    tableWrapper(manta_normal_selected[rank_score>1])
  } 
}
...
try( if (exists('manta_tumor_selected')) {
  ...
  cat('Tumor SV files:',paste(manta_tumor_file,collapse = ' '))
  cat('\n')
...
if (length(mutect2_file)>0) {
  ...
  if (nrow(selection)>0) {  
    protein_position=selection$Protein_position
    for (i in 1:nrow(selection)) protein_position[i] <- strsplit(protein_position[i],'/')[[1]][1]
    protein_position[is.na(protein_position)]=''
