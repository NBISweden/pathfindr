...
# control freec files
freec_Tratio_file <- files[grep(pattern = 'T.hg38.pileup.gz_ratio.txt',files)][1]
freec_Nratio_file <- files[grep(pattern = 'T.hg38.pileup.gz_normal_ratio.txt',files)][1]
freec_Tbaf_file <- files[grep(pattern = 'T.hg38.pileup.gz_BAF.txt',files)][1]
freec_Nbaf_file <- files[grep(pattern = 'B.hg38.pileup.gz_BAF.txt',files)][1]
if (is.na(freec_Nbaf_file)) 
  freec_Nbaf_file <- files[grep(pattern = 'N.hg38.pileup.gz_BAF.txt',files)][1]
freec_info_file <- files[grep(pattern = 'hg38.pileup.gz_info.txt',files)][1]
freec_cnv_file <- files[grep(pattern = glob2rx('*T.hg38.pileup.gz_CNVs'),files)][1]
# Ascat files
...
vep_Tstruct_file <- grep(pattern = glob2rx("*SnpEff/Manta*vs*somaticSV.*ann*.vcf"),files,value = T)[1]
vep_Nstruct_file <- grep(pattern = glob2rx("*SnpEff/Manta*vs*diploidSV.*ann*.vcf"),files,value = T)[1]
...
if (length(mutect2_file)==0)
  mutect2_file <- grep(pattern = glob2rx("*Annotation/VEP/mutect2_*.vcf.vep.ann*.vcf"),files,value = T)[1] # <- bianca
...
if (length(strelka_snv_file)==0)
  strelka_snv_file <- grep(pattern = glob2rx("*Annotation/VEP/Strelka_*_somatic_snvs.vcf.vep.ann*.vcf"),files,value = T)[1] # <- bianca
...
if (length(strelka_indel_file)==0)
  strelka_indel_file <- grep(pattern = glob2rx("*Annotation/VEP/Strelka_*_somatic_indels.vcf.vep.ann*.vcf"),files,value = T)[1] # <- bianca
...
if (length(freec_Tratio_file)>0 &
  ...
  for (i in 1:nrow(cnvs)) {
    all=allgenes[chr==cnvs$chr[i] & start<cnvs$end[i] & end>cnvs$start[i],name]
  ...
  htmlTable(cnvs, col.rgroup = c("none", "#F7F7F7")) #%>%
...
if (length(vep_Tstruct_file)>0) {
  ...
  ix=key %in% btb_hg38_manta_pass$name 
  ix=btb_hg38_manta_pass$name %in% key 
  ix=key %in% btb_hg38_manta_all$name 
  ix=btb_hg38_manta_all$name %in% key
  ...
  manta_tumor_table <- merge(tstruc,snpEff_table,by='N')[order(cumstart),-1]
  
  # Deletion: remove deletions >3MB for "xxx_ablation" consequences
  del_rm <- intersect(which(abs(manta_tumor_table$SVLEN)>3e6),
                      grep('ablation',manta_tumor_table$Annotation,fixed))
  # Amplification: remove >5M for 'duplication' consequences, report census genes
  amp_rm <- intersect(which(abs(manta_tumor_table$SVLEN)>5e6),
                      grep('duplication',manta_tumor_table$Annotation))
  keep=rep(T,nrow(manta_tumor_table)); keep[-union(del_rm,amp_rm)]=F
  
  ## Make new table with predicted effects
  selection <- unique(
    manta_tumor_table[keep,
                      .(SVTYPE,ID=THISID,chr,start,end,altchr,altpos,SVLEN=unlist(SVLEN),
                        Allele,Gene_Name,Annotation,Annotation_Impact,
  ...
  selection[Gene_Name %in% tumorgenes$`Gene Symbol` | Gene_Name %in% local_tumorgenes$V1]$Gene_Tier <- 2
  selection[Gene_Name %in% tumorgenes[Tier==1]$`Gene Symbol`]$Gene_Tier <- 1
  ...
  for (i in selection[,.I]) {
    ...
    if (any(symbol %in% tumorgenes[Tier==1,`Gene Symbol`])) selection$Gene_Tier[i]=1
  ...
  htmlTable(manta_tumor_selected, col.rgroup = c("none", "#F7F7F7")) #%>%
  #kable_styling(bootstrap_options = "striped",position = "left") %>%
...
if (length(vep_Nstruct_file)>0) {
  ...
  manta_normal_table <- merge(nstruc,snpEff_table,by='N')[order(cumstart),-1]
  
  # Deletion: remove deletions >3MB for "xxx_ablation" consequences
  del_rm <- intersect(which(abs(manta_normal_table$SVLEN)>3e6),
                      grep('ablation',manta_normal_table$Annotation,fixed))
  # Amplification: remove >5M for 'duplication' consequences, report census genes
  amp_rm <- intersect(which(abs(manta_normal_table$SVLEN)>5e6),
                      grep('duplication',manta_normal_table$Annotation))
  keep=rep(T,nrow(manta_normal_table)); keep[-union(del_rm,amp_rm)]=F
  
  ## Make new table with predicted effects
  selection <- unique(
    manta_normal_table[keep,
                       .(SVTYPE,ID=THISID,chr,start,end,altchr,altpos,SVLEN=unlist(SVLEN),
                         Allele,Gene_Name,Annotation,Annotation_Impact,
                         Gene_Tier=3,Effect_Tier=3,LOH='',Comment='',
                         REF,ALT=unlist(ALT))]
  ...
  htmlTable(manta_normal_selected, col.rgroup = c("none", "#F7F7F7")) #%>%
  #kable_styling(bootstrap_options = "striped",position = "left") %>%
  #column_spec(7, width = "20em") %>%
  #column_spec(9, width = "20em") %>%
  #scroll_box(width = "1400px", height = "600px")
...
if (exists('tstruc')) {
  ...
  temp=unique(manta_normal_table[THISID %in% manta_normal_selected[Gene_Tier<3,ID],.(cumstart,altcumpos,SVTYPE,IMPRECISE)])
  if (nrow(temp)>0) g=g+geom_curve(aes(x = cumstart,xend = altcumpos,y=-1.499,yend=-1.5,col=SVTYPE,linetype=IMPRECISE),
                                   data = temp,alpha=1/2,
                                   stat = "identity",curvature=-0.2,size=0.5,
                                   position = "identity", angle = 90, ncp = 10,
  ...
  temp=allgenes[name %in% manta_tumor_selected$SYMBOL[manta_tumor_selected$Gene_Tier<3]]
  if (nrow(temp)>0) g=g+geom_label_repel(mapping = aes(x = cumstart,y = 1.45,label=name),
                                         data = temp,
                                         size=3)
  # add NORMAL data afflicted cosmic genes
  temp=allgenes[name %in% manta_normal_selected$SYMBOL[manta_normal_selected$Gene_Tier<3]]
...
if (length(mutect2_file)>0) {
  ...
  selection <- mutect2_table[
    keep,
    .(VARIANT_CLASS,chr,start,end,SYMBOL,Protein_position,Amino_acids,Gene_Tier=3,Effect_Tier=3,LOH='',Consequence,IMPACT,CLIN_SIG,PolyPhen,SIFT,
      MOTIF_NAME, MOTIF_POS, HIGH_INF_POS, MOTIF_SCORE_CHANGE,
      ref,alt,DP,AD,DPn,ADn,allele1_swegen_count)
    ]
  if (nrow(selection)>0) {
    for (i in 1:nrow(selection)) selection$Protein_position[i] <- strsplit(selection$Protein_position[i],'/')[[1]][1]
    
    # if cancer genes within 100k of variant with no symbol (regulatory), assign (one, by tier) as symbol
    for (i in which(selection$SYMBOL=='')) {
      ...
      if (nrow(near)>0) {
        ...
        selection$SYMBOL[i]=near_cancer[order(Tier),`Gene Symbol`][1]
      }
    }  
  ...
  mutect2_selected <- selection[order(Effect_Tier,Gene_Tier)]
...
if (length(strelka_snv_file) * length(strelka_indel_file)>0) {
  ...
  selection <- strelka_snvs[
    ...
    .(VARIANT_CLASS,chr,start,end,SYMBOL,Protein_position,Amino_acids,Gene_Tier=3,Effect_Tier=3,LOH='',Consequence,IMPACT,CLIN_SIG,PolyPhen,SIFT,
      MOTIF_NAME, MOTIF_POS, HIGH_INF_POS, MOTIF_SCORE_CHANGE,
      ref,alt,DP=DPt,DPn,allele1_swegen_count)
    ]
  if (nrow(selection)>0) {
    for (i in 1:nrow(selection)) selection$Protein_position[i] <- strsplit(selection$Protein_position[i],'/')[[1]][1]
  ...
  strelka_selected <- rbind(strelka_selected,selection)[order(Effect_Tier,Gene_Tier)]
...
if (length(haplotypecaller_N_file)>0) {
  ...
  ix_NinT=match(names(vcf),names(vcf_t))
  g=geno(vcf)
  g_t=geno(vcf_t)
  inf=(info(vcf))
  rr=rowRanges(vcf)
  ...
  # ## Where there is no gene, put those within 100kb
  # t_genes=allgenes[allgenes$name %in% tumorgenes$`Gene Symbol`]
  # for (i in which(haplotypecaller_table$SYMBOL=='')) {
  #   ix=t_genes$cumstart < haplotypecaller_table$start[i] + 100000 &
  #     t_genes$cumend > haplotypecaller_table$start[i] - 100000
  #   if (sum(ix)>0) haplotypecaller_table$SYMBOL[i]=paste(t_genes$name[ix],collapse = ' ')
  # }
  ...
  selection <- haplotypecaller_table[keep_haplotypecaller,
                                     .(VARIANT_CLASS,chr,start,end,SYMBOL,Protein_position,Amino_acids,Gene_Tier=3,Effect_Tier=3,LOH='',
                                       Consequence,IMPACT,CLIN_SIG,PolyPhen,SIFT,
                                       MOTIF_NAME, MOTIF_POS, HIGH_INF_POS, MOTIF_SCORE_CHANGE,
                                       ref,alt,alt2,DP,DP_t,RD,RD_t,AD,AD_t,AD2,AD2_t,allele1_swegen_count,allele2_swegen_count)]
...
chromplot=function(thischr) {
  ...
  if (exists('tstruc')) if (nrow(tstruc)>0) {
    ...
    if (nrow(temp)>0) g=g+geom_curve(aes(x = start,xend = altpos,y=0.499,yend=0.5,col=SVTYPE,linetype=IMPRECISE),
                                     data = temp,alpha=1/2,
    ...
    temp=tstruc[tstruc$chr==thischr & tstruc$FILTER=='PASS' & tstruc$altchr!=tstruc$chr,]
    if (nrow(temp)>0) { 
      g=g+geom_label_repel(data = temp,mapping = aes(x=start,y=0.5,label=alt),
