...
vep_Tstruct_file <- grep(pattern = "^.*SnpEff/Manta.*vs.*somaticSV\\..*ann.*\\.vcf$",files,value = T)[1]
vep_Nstruct_file <- grep(pattern = "^.*SnpEff/Manta.*vs.*diploidSV\\..*ann.*\\.vcf$",files,value = T)[1]

# Haplotype caller variant files
haplotypecaller_T_file <- grep(pattern = "^.*VEP/haplotypecaller.*[TR]\\.vcf\\.vep\\.ann\\.vcf$",files,value = T) # local
haplotypecaller_N_file <- grep(pattern = "^.*VEP/haplotypecaller.*[BN]\\.vcf\\.vep\\.ann\\.vcf$",files,value = T) # local
...
strelka_snv_file <- grep(pattern = glob2rx("*/Annotation/Strelka_*somatic_snvs.snpeff.ann.canon.vep.ann.pick.PASS.recode.vcf"),files,value = T) # <- local
...
strelka_indel_file <- grep(pattern = glob2rx("*/Annotation/Strelka_*somatic_indels.snpeff.ann.canon.vep.ann.pick.PASS.recode.vcf"),files,value = T) # <- local
...
```{r}
snptable=fread('~/reports/swegen_snp_counts.csv') #  swegen_hg38_allele_counts
setkey(snptable,name)
swegen_hg38_manta_all=fread('~/reports/swegen_sv_counts.csv')
chrsz=fread('~/reports/chr_data.csv')
allgenes=fread('~/reports/genes_ensemble_canonical.csv')
...
if (length(vep_Tstruct_file)>0) {
  ...
  ix=key %in% swegen_hg38_manta_pass$name 
  ix=swegen_hg38_manta_pass$name %in% key 
  ix=key %in% swegen_hg38_manta_all$name 
  ix=swegen_hg38_manta_all$name %in% key
  # thus 1,5k reamins using "all" and 2,5k remains using "pass".
  # now using 2 observations as cutoff 
  ix <- key %in% swegen_hg38_manta_all[value>1]$name
  ## do the filter
  tstruc <- tstruc[!ix]
...
if (length(vep_Nstruct_file)>0) {
  ...
  # now using 2 observations as cutoff 
  ix <- key %in% swegen_hg38_manta_all[value>1,name]
  # 390 of 10k remain with test sample
  ## do the filter
  nstruc <- nstruc[!ix]
...
if (length(mutect2_file)>0) {
  ...
  selection <- mutect2_table[
    keep,
    .(VARIANT_CLASS,chr,start,end,Swegen_count,SYMBOL,Protein_position,Amino_acids,Gene_Tier=3,Effect_Tier=3,LOH='',Consequence,IMPACT,CLIN_SIG,PolyPhen,SIFT,
  ...
  if (nrow(selection)>0) {
    ...
    for (i in which(selection$SYMBOL=='')) {
      ...
      if (nrow(near)>0) {
        ...
        if (nrow(near_cancer)>0) selection$SYMBOL[i]=near_cancer[order(Tier),`Gene Symbol`][1]
      }
    }  
    ...
    # Mutation effect: T2 (what?). T1 for MSK hotspot, or HIGH-impact in TSG, or Clinvar pathogenic
    selection$Effect_Tier[selection$IMPACT %in% c('HIGH','MODERATE')] <- 2
...
if (length(strelka_snv_file) * length(strelka_indel_file)>0) {
  ...
  strelka_snvs=strelka_snvs[Swegene_count<3]
  ...
  strelka_indels=strelka_indels[Swegen_count<3]
...
if (length(haplotypecaller_N_file)>0) {
  ...
  in_ref=snptable[names(vcf)]
  in_ref$value[is.na(in_ref$value)]=0
  vcf=vcf[in_ref$value<3]  # 32000 kvar, bättre än de 97k som fanns kvar med alt 1 (och <2)
  ...
  # ## Before adding the (LARGE) info and annotation fields, filter with Swegen <-------  a better solution now applies.
  # # Make 2 key sets: key1 and key2. If variant has 2 alt alleles, remove if both alleles satisfy criteria.
  ...
  # nmuts <- nmuts[!remove,] # filtered.  # 97k variants remained...
  ...
  selection <- haplotypecaller_table[keep_haplotypecaller,
                                     .(VARIANT_CLASS,chr,start,end,Swegen_count,SYMBOL,Protein_position,Amino_acids,Gene_Tier=3,Effect_Tier=3,LOH='',
...
### Combined data {.active}
```{r}
## Combine all data into a preliminary summary of gene effects
