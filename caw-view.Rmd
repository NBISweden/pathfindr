    ...
    if (length(all)>0) { # if any genes
      ...
      if (nrow(genes)>0) { # if any of our cancer genes
        ...
        if (any(genes$Tier==1,na.rm = T)) {
          cnvs$rank_score[i]=3
          cnvs$rank_terms[i]='cosmic_T1'
        } else if (any(genes$Tier==2,na.rm = T)) {
          cnvs$rank_score[i]=2
          cnvs$rank_terms[i]='cosmic_T2'
        } else {
          cnvs$rank_score[i]=2
...
#### Scatter plot
<!-- Scatter plot -->
```{r, echo=FALSE,fig.width=15,fig.height=8,warning=F}
if (exists('binned')) {
...
if (length(vep_Tstruct_file)>0) {
  ...
  if (nrow(tstruc)>0) {
    ...
    keep=rep(T,nrow(manta_tumor_table)) 
    keep[union(del_rm,amp_rm)]=F
    ...
    manta_tumor_selected <- data.table()
  }
...
if (length(vep_Nstruct_file)>0) {
  ...
  # manipulate into a data frame with relevant data, "info"" annotations also need to be added
  nstruc=cbind(data.table(THISID=names(rr)),as.data.table(rr),as.data.table(inf))[FILTER=='PASS']
  ...
  keep=rep(T,nrow(manta_normal_table)) 
  keep[union(del_rm,amp_rm)]=F
  ...
  for (i in selection[,.I]) {
    ...
    if (nrow(gene)>0) {
      # first fusions/translocations
      if (length(grep('fusion',selection$Annotation[i]))>0) {
        g=grep('fusion',gene$`Role in Cancer`)
        if (length(g)>0) { # this is a fusion and either end is fusion gene yes
          selection$Effect_Tier[i]=1 # then at least tier 1/2?
          selection$Comment[i]='partial_known_fusion'
        }    
        # if perfect match with Cosmic
        if (nrow(gene)>1) if (as.logical(grep(gene$`Gene Symbol`[2],gene$`Translocation Partner`[1]))) {
          selection$Effect_Tier[i]=1 # common translocation/fusion
          selection$Comment[i]='known_fusion'
        } 
...
if (length(mutect2_file)>0) {
  ...
  selection <- mutect2_table[
    keep,
    .(VARIANT_CLASS,Existing_variation,chr,start,end,Swegen_count,SYMBOL,Protein_position,Amino_acids,Gene_Tier=3,Effect_Tier=3,LOH='',Consequence,IMPACT,CLIN_SIG,PolyPhen,SIFT,
  ...
  if (nrow(selection)>0) {
    ...
    selection$Gene_Tier[selection$SYMBOL %in% local_tumorgenes$Gene | selection$SYMBOL %in%  tumorgenes$`Gene Symbol`] <- 2
...
if (length(haplotypecaller_N_file)>0) {
  ...
  selection <- haplotypecaller_table[keep_haplotypecaller,
                                     .(VARIANT_CLASS,Existing_variation,chr,start,end,Swegen_count,SYMBOL,Protein_position,Amino_acids,Gene_Tier=3,Effect_Tier=3,LOH='',
...
```{r}
## Combine all T1/2 data into a pretty table
combined=NULL
