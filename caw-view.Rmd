...
```{r}
  ...
  sampleList <- list(
    nameHere=list(
      ...
      cnvs,
      ascat_cnv,
      manta_normal_selected,#manta_normal_table,
      manta_tumor_selected,#manta_tumor_table,
      mutect2_selected,#mutect2_table,
      strelka_selected,#strelka_indels,strelka_snvs,
      haplotypecaller_selected#,haplotypecaller_table # >2GB with the bid tables added
    )
  )
...
chromplot=function(thischr) {
  ...
  if (exists('tstruc')) {
    ...
    thischrstart=chrsz$starts[chrsz$chr==thischr]
  ...
  if (exists('nbaf')) {
    ...
    g=g+geom_point(aes(x=Position,y=NBAF -1.5),data = ascat_nbaf[ascat_nbaf$Chr==thischr,],alpha=1/8,shape='.')
  }
  return(g)
}
```
