...
cat('Sample:',basename(getwd()),'\tDirectory:',getwd(),'\tDate:',date())
```
...
if (length(vep_Tstruct_file)>0) {
  ...
  kable(manta_tumor_selected, "html") %>%
  kable_styling(bootstrap_options = "striped",position = "left") %>%
...
if (length(vep_Nstruct_file)>0) {
  ...
  kable(manta_normal_selected, "html") %>%
  kable_styling(bootstrap_options = "striped",position = "left") %>%
  column_spec(7, width = "20em") %>%
  column_spec(9, width = "20em") %>%
  scroll_box(width = "1400px", height = "600px")
...
if (exists('tstruc')) {
  ...
  g=g+geom_curve(aes(x = cumstart,xend = altcumpos,y=1.499,yend=1.5,col=SVTYPE,linetype=IMPRECISE),
                 data = unique(manta_tumor_table[,.(cumstart,altcumpos,SVTYPE,IMPRECISE)]),alpha=1/2,
  ...
  g=g+geom_curve(aes(x = cumstart,xend = altcumpos,y=-1.499,yend=-1.5,col=SVTYPE,linetype=IMPRECISE),
                 data = unique(manta_normal_table[THISID %in% manta_normal_selected[Gene_Tier<3,ID],
                                                  .(cumstart,altcumpos,SVTYPE,IMPRECISE)]),alpha=1/2,
                 stat = "identity",curvature=-0.2,size=0.5,
                 position = "identity", angle = 90, ncp = 10,
                 arrow = arrow(length=unit(0.10,"cm"), ends="both", type = "open"), lineend = "butt", na.rm = FALSE, show.legend = NA,
                 inherit.aes = FALSE)
  # add TUMOR data afflicted cosmic genes
  g=g+geom_label_repel(mapping = aes(x = cumstart,y = 1.45,label=name),
                      data = allgenes[name %in% manta_tumor_selected$SYMBOL[manta_tumor_selected$Gene_Tier<3]],
                      size=2)
  # add NORMAL data afflicted cosmic genes
  g=g+geom_label_repel(mapping = aes(x = cumstart,y = -1.55,label=name),
                      data = allgenes[name %in% manta_normal_selected$SYMBOL[manta_normal_selected$Gene_Tier<3]],
                      size=2)
...
if (length(haplotypecaller_N_file)>0) {
  ...
  nmuts$type[nchar(nmuts$type)>3]='other'
  ...
  # And certain effects
  regulatory=c(grep('TF',haplotypecaller_table$Consequence),grep('regulatory',haplotypecaller_table$Consequence))
  
  keep <- sort(unique(c(impact,clinvar_pathogenic,polyphen_damaging,sift_deleterious,regulatory)))
  selection <- haplotypecaller_table[keep,
    .(VARIANT_CLASS,chr,start,end,SYMBOL,Protein_position,Amino_acids,Gene_Tier=3,Effect_Tier=3,LOH='',Consequence,IMPACT,CLIN_SIG,PolyPhen,SIFT,
  ...
  kable(haplotypecaller_selected, "html") %>%
  kable_styling(bootstrap_options = "striped",position = "left") %>%
  scroll_box(width = "1400px", height = "600px")
...
```{r}
  ...
  writeVcf(obj = vrA[vrA %in% vrB],filename = paste0(sampleData$name,'_forPatterns.vcf'))
  #
  ...
  save(sampleList,file=paste0(sampleData$name,'_data.Rata'))
...
chromplot=function(thischr) {
  ...
  if (exists('tstruc')) {
    ...
    temp=tstruc[tstruc$chr==thischr & tstruc$FILTER=='PASS' & tstruc$altchr!=tstruc$chr,]
    if (nrow(temp)>0) { 
      # g=g+geom_curve(aes(x = start,xend = start,y=1.499,yend=1.5,col=SVTYPE,linetype=IMPRECISE),
      #                                data = temp,alpha=1/2,
      #                                curvature=-0.2,size=1,
      #                                position = "identity", angle = 90, ncp = 50,
      #                                arrow = arrow(length=unit(0.10,"cm"), ends="both", type = "open"), lineend = "butt", na.rm = FALSE, show.legend = NA,
      #                                inherit.aes = FALSE)
      g=g+geom_label_repel(data = temp,mapping = aes(x=start,y=1,label=alt),
    ...
    if (exists('haplotypecaller_table')) { 
      g=g+
        geom_point(aes(x=start,y=AF_t -1.5,fill=type,shape=type),
                   data = haplotypecaller_table[chr==thischr],alpha=1)+
        geom_label_repel(data = haplotypecaller_table[Amino_acids!='',],
    ...
    #genes!
    temp=allgenes[name %in% tumorgenes$`Gene Symbol`]
    g=g+geom_label_repel(mapping = aes(x = (cumstart+cumend)/2,y =  -2.5,label=name),
                         data=allgenes[name %in% tumorgenes$`Gene Symbol`],
                         nudge_y= -0.1,size=3)
    return(g)
}
```
