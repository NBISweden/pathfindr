...
vep_Tstruct_file <- grep(pattern = glob2rx("*VEP/Manta*vs*somaticSV.*ann.vcf"),files,value = T)
vep_Nstruct_file <- grep(pattern = glob2rx("*VEP/Manta*vs*diploidSV.*ann.vcf"),files,value = T)
...
if (exists('binned')) {
  ...
  g=ggplot()+expand_limits(y=c(.5,1))+expand_limits(x=c(.5,1.5))+
...
## Structural variants {.active .tabset}
...
if (length(haplotypecaller_N_file)>0) {
  ...
  # And certain effects
  regulatory=c(grep('TF',haplotypecaller_table$Consequence),grep('regulatory',haplotypecaller_table$Consequence)) # <- not used below
  
  keep_haplotypecaller <- sort(unique(c(impact,clinvar_pathogenic,polyphen_damaging,sift_deleterious))) #,regulatory
...
writeVcf(obj = vrA[vrA %in% vrB],filename = paste0('~/reports/',sampleData$name,'_forPatterns.vcf'))
#
