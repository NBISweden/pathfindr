...
vep_Tstruct_file <- grep(pattern = glob2rx("*VEP/Manta*vs*somaticSV.*ann*.vcf"),files,value = T)[1]
vep_Nstruct_file <- grep(pattern = glob2rx("*VEP/Manta*vs*diploidSV.*ann*.vcf"),files,value = T)[1]
...
if (length(vep_Tstruct_file)>0) {
  ...
  # for (i in 1:nrow(tstruc)) { # for each variant
  #   # for each VEP annotation:
  #   for (j in 1:length(tstruc$ANN[[i]])) { 
  ...
  #   }
  # }
  # snpEff_table=as.data.table(snpEff_table)
