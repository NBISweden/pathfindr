...
if (exists('tstruc')) {
  ...
  g=g+geom_curve(aes(x = cumstart,xend = altcumpos,y=0.499,yend=0.5,col=SVTYPE,linetype=IMPRECISE),
                 data = manta_tumor_table,alpha=1/2,
  ...
  g=g+geom_curve(aes(x = cumstart,xend = altcumpos,y=-0.499,yend=-.5,col=SVTYPE,linetype=IMPRECISE),
                 data = manta_normal_table,alpha=1/2,
                 stat = "identity",curvature=-0.2,size=1,
                 position = "identity", angle = 90, ncp = 10,
                 arrow = arrow(length=unit(0.10,"cm"), ends="both", type = "open"), lineend = "butt", na.rm = FALSE, show.legend = NA,
                 inherit.aes = FALSE)
